...
import com.kodilla.stream.portofolio.Board;
import com.kodilla.stream.portofolio.Task;
import com.kodilla.stream.portofolio.TaskList;
import com.kodilla.stream.portofolio.User;
...
public class BoardTestSuite {
    ...
    public void testAddTaskListAverageWorkingOnTask() {
        ...
        countOfDays = project.getTaskLists().stream()
            .filter(inProgressTasks::contains)
            .flatMap(taskList -> taskList.getTasks().stream())
            .map(task -> task.getCreated())
        ...
        countOfTasks = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(taskList -> taskList.getTasks().stream())
                .count();

        averageDayCount = countOfDays / countOfTasks;

        //Then
        Assert.assertEquals(30, countOfDays);
